...
import { ChallengeDataType, RowItem } from '../Components/RowItem';

declare type DataType = { current: number, target: number, details: Array<{ day: Date, status: number | string }> } & ChallengeDataType
...
export const Detail = ({ route, navigation }) => {
  ...
  }, [route.params, useDectectDataChanged(challengeRepository)]);
  return <Background style={[{}]}>
    <View style={{ padding: 16, paddingBottom: 0 }}>
      <Header data={data} />
      <RowItem challenge={data} style={[{ borderRadius: 10, borderColor: colors.primary }]} />
    </View>
...
const loadData = async (id): Promise<DataType> => {
  const challenge = await challengeRepository.findOne(c => c.id == id);
  const current = moment(new Date).diff(challenge.start, 'days');
  const target = moment(challenge.end).diff(challenge.start, 'days');
  const details = [];
  let percentage: 0;
  const total = moment(challenge.end).diff(challenge.start, 'days');
  //challenge --> type {link times, target}

  return { percentage, total, current, target, details, ...challenge };
};
  ...
  return (<View>
    <View style={[{ height: HEADER_HEIGHT }]}>
      ...
      <TouchableOpacity
        ...
        onPress={() => Router.Open(navigation, 'ChallengerApp', { screen: 'Edit', id: props.data.id })}            >
