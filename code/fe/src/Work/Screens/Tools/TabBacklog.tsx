...
export const TabBackLog = () => {
    ...
    const data = useAsyncAction(async () => {
        return (await workRepository.filter(w => !w.startDate));
    }, [useDectectDataChanged(workRepository)], []);

    const emptyList = () => {
        return <View style={{ alignItems: 'center' }}>
            <TouchableOpacity onPress={onPlusClick} style={{ padding: 20 }}><Text style={{ fontSize: FONTSIZE.NORMAL }}>{t('Nhấn ')}<Text style={{ color: colors.colorLink, fontSize: FONTSIZE.NORMAL }}>{t('+')}</Text><Text style={{ fontSize: FONTSIZE.NORMAL }}>{t(' để thêm nhiệm vụ mới.')}</Text></Text>
            </TouchableOpacity>
        </View>;
    };
