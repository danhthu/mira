...
File nằm ở `growth/content/vi/01-*.md` đến `28-*.md`. Hàng đợi publish ở
`growth/content/content-queue.json` (28 dòng, `status: "pending"` hết — chưa bài nào live).
...
**Trạng thái file**: bản site (`growth/blog/src/content/blog/`) có ngày 29-88. Ngày 89-91 mới chỉ
có bản draft trong `growth/content/vi/`, chưa đưa sang site. `content-queue.json` đã có đủ 91 dòng,
...
- **Ngày 89-91**: đưa bản draft sang `growth/blog/src/content/blog/` để khép trọn tuần 13.
...
- Chạy `check-posts` (đối chiếu frontmatter với bảng, khung số từ, có phép tính, kết bằng câu hỏi,
  danh sách từ cấm) trước khi mirror sang `growth/content/vi` và trước khi build.
...
F:\Danh Thu\github\mira\
  CLAUDE.md                  ← index gốc, trỏ vào code/CLAUDE.md và growth/
  ...
  growth\
    ...
    content\
      HANDOFF.md              ← file này (trước là BLOG-HANDOFF.md ở root)
      content-queue.json       ← hàng đợi publish
      vi\01-*.md ... 91-*.md    ← bài draft
```
