...
**App (app/)**: React Native + Expo (SDK mới nhất), TypeScript strict, SQLite qua `expo-sqlite` + Drizzle ORM, Zustand, Vitest cho logic thuần.
...
```
app/src/features/<name>/   ← mỗi feature là một folder độc lập
app/src/shared/            ← thứ DUY NHẤT được import giữa các feature
app/src/core/              ← hàm thuần, không phụ thuộc React, phải có test
app/src/db/                ← SQLite schema + Drizzle migrations + repositories
app/src/i18n/vi.ts         ← mọi chuỗi hiển thị, không inline string trong JSX
app/src/store/             ← Zustand (không Redux)
```
...
- Mọi hằng số vào `src/core/constants.ts` (app) hoặc `be/src/shared/constants.ts` (be). Không số ma thuật trong code.
